...
<x-employer.main-layout heading="Edit Job">
    ...
    @push('scripts')
        <script>
            const editJobDataRoute = "{{ route('emp.job.get.edit') }}"
            const updateJobRoute = "{{ route('emp.job.update') }}"
            const goBackRoute = "{{ route('emp.posted.jobs') }}"
        </script>
        <script src="{{ asset('js/custom/loader.js') }}"></script>
        <script src="{{ asset('js/employer/edit-job.js') }}"></script>
    @endpush
