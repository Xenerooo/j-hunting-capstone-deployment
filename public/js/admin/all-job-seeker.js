function getSeeker(searchQuery = "", sortBy = "all") {
    $.ajax({
        url: getSeekerRoute,
        type: "GET",
        data: {
        ...
        success: function (response) {
            const seekersData = response.data;

            $("#table-body").empty();
            $("#message-container").empty();

            if (!seekersData.length) {
            ...
            seekersData.forEach((data) => {
                ...
                const viewSeeker = `/admin/all-job-seekers/view/${data.seeker_id}`;
                ...
                const template = `
                      <tr class="bg-gray-100 border-b border-gray-200">
                                  ...
                                  <div>
                                      ...
                                      <div class="flex items-center space-x-1">
                                              <p class="text-gray-800 flex items-center">
                                                  ...
                                                  <span class="text-[12px]">Job Seeker has <strong>${jobTypeCount}</strong> job types</span>
                                              </p>
                                          </div>
                                  </div>
                              </div>
                          ...
                          <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap text-center">
                              <p class="text-xs text-yellow-600 px-2 py-1 rounded-xl">${
                                  data.account.email
                              }</p>
                          </td>
...
function downloadExcel() {
    $("#downloadExcel").on("click", function () {
        ...
        const url = new URL(getSeekerDownloadRoute);
