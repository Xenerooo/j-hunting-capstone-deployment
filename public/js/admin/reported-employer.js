function ignoreReport(reportId, employerId) {
    if (!reportId || !employerId) {
        alert("Invalid report or employer ID.");
        return;
    }
...
function showReportedEmployer(searchQuery = "", sortBy = "all") {
    $.ajax({
        type: "get",
        url: showReportedEmployerRoute,
        data: {
            search: searchQuery,
            sort: sortBy,
        },
        ...
        success: function (response) {
            ...
            report.forEach((data) => {
                ...
                const tagName =
                    data.employer.employer_type == "company"
                        ? data.employer.comp_name
                        : fullName;

                const viewEmployer = `/admin/reported-employers/view/${data.employer_id}`;
                    ...
                    <div class="group bg-white/85 backdrop-blur-sm border border-rose-100 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                        <div class="p-5">
                            <div class="flex items-start gap-4">
                                ...
                                <div class="min-w-0">
                                    <p class="text-sm font-semibold text-gray-900 truncate" name="employer_name">${tagName}</p>
            ...
            $(".ignore-button")
                ...
                .on("click", function () {
                    ...
                    ignoreReport(reportId, employerId);
                });
        },
        error: function (xhr) {
