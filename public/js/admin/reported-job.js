function ignoreReport(reportId, jobId) {
    console.log(reportId);

    if (!reportId || !jobId) {
        alert("Invalid report or job ID.");
        return;
    }
...
function showReportedJob(searchQuery = "", sortBy = "all") {
    $.ajax({
        type: "get",
        url: showReportedJobRoute,
        data: {
            search: searchQuery,
            sort: sortBy,
        },
        ...
        success: function (response) {
            ...
            data.forEach((data) => {
                ...
                const viewJob = `/admin/reported-jobs/view/${data.job_id}`;
                ...
                if (Array.isArray(data.reports) && data.reports.length > 0) {
                    reportsHtml = data.reports
                        .map(
                            (report) =>
                                `<div class="mb-1 text-rose-700 text-start font-semibold text-sm"><span>${
                    ...
                    reportedAt = data.reports[0]?.reported_at;
                } else {
                    reportsHtml = `<div class="text-gray-500 text-xs">(No reports found)</div>`;
                }
                ...
                const template = `
                  <div class="group bg-white/85 backdrop-blur-sm border border-rose-100 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                      <div class="p-5">
                          ...
                          <div class="space-y-2">
                              <span class="text-base font-semibold text-gray-900">${data.title}</span>
                              <div class="description-wrapper relative overflow-hidden max-h-[80px]">${reportsHtml}</div>
                          </div>
                          <div class="mt-5 grid grid-cols-2 gap-2">
                              ...
                              <button class="ignore-button inline-flex items-center justify-center rounded-xl bg-white text-rose-700 font-semibold px-4 py-2.5 text-sm border border-rose-300 hover:bg-rose-50 transition" data-report-id="${reportId}" data-job-id="${data.job_id}">
                                  Ignore Report
                              </button>
                          </div>
                      </div>
